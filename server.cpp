...
int main(int argc, char* argv[])
{
    ...
    while(1)
    {
        ...
        {
            if((fds[i].fd == listenfd)&&(fd[i].revent  & POLLIN))
            {
                ...
                fds[user_counter].events =POLLIN | POLLRD00HUP | POLLERR ;
            ...
            else if (fds[i].revent & POLLRDHUP)
            {
            ...
            else if (fds[i].revent & POLLIN)
            {
                ...
                cout<<"get"<<ret<< "byte of client data"<<users[connfd].buf<<"from"<<connfd<<endl;
                if(ret <0)
                {
                    if(error != EAGAIN)
                    {
